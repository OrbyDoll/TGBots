...
async def callback_message(call: types.CallbackQuery, state: FSMContext):
    ...
    try:
        if "buy_" in call.data:
            ...
            await bot.send_message(
                chatid,
                "Пришлите ник человека, которого вы хотите разбанить. Для отмены введите '-' без кавычек",
            )
            await state.set_state(ClientState.UNBAN)
            ...
            await bot.send_message(
                chatid,
                "Выберете категорию интересующего товара📋",
