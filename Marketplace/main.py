...
async def callback_message(call: types.CallbackQuery, state: FSMContext):
    ...
    if 'create_offer' in call.data:
        await bot.send_message(chatid, 'К какой категории относиться ваш товар?', reply_markup=nav.categor)
        ...
        if int(call.data.split()[1]) == chatid:
            ...
            await bot.send_message(chatid, f'Недостаточно денег на балансе. Ваш баланс {db.get_user(chatid)[1]} USDT')
            await state.set_state(ClientState.START)
            return
        print(int(call.data.split()[3]))
        print(balance, 'bal')
        await state.update_data(seller_id=call.data[4:])
        await bot.send_message(chatid, 'Вы уверены что хотите купить этот товар? Потом это действие нельзя будет отменить', reply_markup=nav.buy_choose)
    elif 'accept' in call.data:
        state_data = await state.get_data()
        buy_link = state_data['seller_id']
        await bot.edit_message_text(chat_id=chatid, text=f'Отправьте данный текст нашему гаранту для начала сделки: `{nav.code_link(buy_link.encode())}`',message_id=call.message.message_id, parse_mode="MARKDOWN", reply_markup=types.InlineKeyboardMarkup().add(
                types.InlineKeyboardButton(text='Перейти в гарант бота', url='https://t.me/pradagarantbot')
            ))
        await state.set_state(ClientState.START)
        ...
        await state.set_state(ClientState.START)
        await bot.send_message(chatid, "Выберите действие", reply_markup=nav.choose_action)
    ...
    for offer in user_offers:
        if not offer == '':
            ...
            if choosed_category == product_category:
                    await bot.send_message(message.chat.id, f'Категория: {product_category}\nТовар: {product_name}\nЦена: {product_price} USDT', reply_markup=nav.get_offer_del_button(del_link))
    ...
    for user_offers in all_offers:
        ...
        for offer in user_offers_list:
            if not offer == '':
                ...
                if choosed_category == product_category:
                    await bot.send_message(message.chat.id, f'Категория: {product_category}\nТовар: {product_name}\nАвтор: {owner_id}\nЦена: {product_price} USDT', reply_markup=nav.get_offer_buy_button(buy_link))

@dp.message_handler(state=ClientState.CREATEOFFER_CATEGORY)
async def createOffer(message: types.Message, state: FSMContext):
    await state.update_data(product_category=message.text)
    await bot.send_message(message.chat.id, 'Напишите краткое описание вашего товара')
    await state.set_state(ClientState.CREATEOFFER_NAME)

@dp.message_handler(state=ClientState.CREATEOFFER_NAME)
async def createOffer(message: types.Message, state: FSMContext):
    await state.update_data(product_name=message.text)
    await bot.send_message(message.chat.id, 'Напишите стоимость вашего товара в USDT')
...
async def createOffer(message: types.Message, state: FSMContext):
    try:
        ...
        await bot.send_message(message.chat.id, 'Продукт успешно добавлен', reply_markup=nav.choose_action)
        await state.set_state(ClientState.START)
