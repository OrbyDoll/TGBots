...
def set_notice_id(user_id, notice_id):
    connection = sqlite3.connect(db)
    q = connection.cursor()
    q.execute("UPDATE users SET notice_id = ? WHERE user_id = ?", (notice_id, user_id))
    # conn.commit()
    # conn.close()


def get_notice_id(user_id):
    connection = sqlite3.connect(db)
    q = connection.cursor()
    res = q.execute("SELECT notice_id FROM users WHERE user_id = ?", (user_id,))
    # conn.commit()
    # conn.close()
    return res
...
def get_notice(user_id):
    connection = sqlite3.connect(db)
    q = connection.cursor()
    res = q.execute("SELECT notice FROM users WHERE user_id = ?", (user_id,)).fetchone()
    # conn.commit()
    # conn.close()
    return res


def increase_notice(user_id):
    connection = sqlite3.connect(db)
    q = connection.cursor()
    prev_notice = int(get_notice(user_id)[0])
    return q.execute(
        "UPDATE users SET notice = ? WHERE user_id = ?",
        (
            prev_notice + 1,
            user_id,
        ),
...
def reset_notice(user_id):
    connection = sqlite3.connect(db)
    q = connection.cursor()
    return q.execute("UPDATE users SET notice = ? WHERE user_id = ?", (0, user_id))
...
def first_join(user_id, username):
    ...
    if row is None:
        q.execute(
            "INSERT INTO users (user_id, offers, balance, ban, nick, notice) VALUES ('%s', '%s', '%s', '%s', '%s', '%s')"
            % (user_id, "0", "0", "0", username, 0)
        )
        connection.commit()
    connection.close()
...
def notice_ban(user_id):
    connection = sqlite3.connect(db)
    q = connection.cursor()
    try:
        q.execute("UPDATE users SET ban = 2 WHERE user_id = ?", (user_id,))
    except:
        pass
    connection.commit()
    connection.close()
