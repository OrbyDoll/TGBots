...
async def sendMessage(message: types.Message, state: FSMContext):
    if message.chat.type == "private":
        if not db.user_exists(message.from_user.id):
            ...
            if not message.from_user.id == cfg.admin_id and not message.from_user.id == cfg.glav_admin_id:
                await bot.send_message(
                    message.chat.id,
                    "Привет! Пишите сообщенияя ниже и наш админ ответит вам в ближайшее время.",
                )
            elif message.from_user.id == cfg.admin_id or message.from_user.id == cfg.glav_admin_id:
                await bot.send_message(
                    message.chat.id,
...
async def cmd_check(message: types.Message, state: FSMContext):
    if message.chat.type == "private":
        if message.from_user.id == cfg.admin_id or message.from_user.id == cfg.glav_admin_id:
            if len(db.get_active_users()) > 0:
                await bot.send_message(message.chat.id, 'Выберите пользователя, сообщения которого вы хотите увидеть', reply_markup=nav.get_users_markup(db))
                await state.set_state(ClientState.GET_MESSAGES)            
            else:
                await bot.send_message(message.chat.id, "На данный момент сообщений нет.")
        else:
...
async def cmd_clear(message: types.Message, state: FSMContext):
    if message.chat.type == "private":
        if message.from_user.id == cfg.admin_id or message.from_user.id == cfg.glav_admin_id:
            if len(db.get_active_users()) > 0:
                await bot.send_message(message.chat.id, 'Выберите пользователя, сообщения которого вы хотите удалить', reply_markup=nav.get_users_markup(db))
...
async def cmd_answer(message: types.Message, state: FSMContext):
    if message.chat.type == "private":
        if message.from_user.id == cfg.admin_id or message.from_user.id == cfg.glav_admin_id:
            if len(db.get_active_users()) > 0:
                await bot.send_message(message.chat.id, 'Выберите пользователя, на сообщение которого вы хотите ответить', reply_markup=nav.get_users_markup(db))
...
async def get_messages(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        print(e)
        await bot.send_message(
            message.chat.id, "Что-то пошло не так. Попробуйте еще раз",reply_markup=nav.get_users_markup(db)
        )
    await state.finish()
...
async def answer_get_id(message: types.Message, state: FSMContext):
    ...
    await bot.send_message(message.chat.id, "Напишите сообщение, которое вы хотите отправить данному пользователю.")
...
async def add_message(message: types.Message):
    if message.chat.type == "private" and (not message.from_user.id == cfg.admin_id or not message.from_user.id == cfg.glav_admin_id):
