...
class Database:
    ...
    def get_auction(self, author_id):
        with self.connection:
            ...
            return res
    def add_auction(self, author_id, product, start_cost):
        with self.connection:
            return self.cursor.execute(
                "INSERT INTO `auctions` (`author_id`, `product`, `start_cost`, `current_cost`) VALUES (?,?,?,?)", (author_id, product, start_cost, start_cost)
            )
    ...
    def check_active_auction(self, author_id):
        with self.connection:
            result = self.cursor.execute(
                "SELECT `auction_id` FROM `auctions` WHERE `author_id` = ?", (author_id,)
            ).fetchall()
            return bool(result)
    def set_start_cost(self, author_id, new_start_cost):
        with self.connection:
            return self.cursor.execute(
                "UPDATE `auctions` SET `start_cost` = ? WHERE `author_id` = ?", (new_start_cost,author_id,)
            )
    ...
    def change_members(self, author_id, factor):
        with self.connection:
            ...
            return self.cursor.execute(
                "UPDATE `auctions` SET `members` = ? WHERE `author_id` = ?", (new_members, author_id,)
            )
    def get_members_id(self, author_id):
        with self.connection:
            res = self.cursor.execute(
                "SELECT `members_id` FROM `auctions` WHERE `author_id` = ?", (author_id,)
            ).fetchone()
            return res
    def set_members_id(self, author_id, new_members_id):
        with self.connection:
            self.cursor.execute(
                "UPDATE `auctions` SET `members_id` = ? WHERE `author_id` = ?", (new_members_id, author_id,)
            )
    def set_auction_status(self, author_id, status):
        with self.connection:
            res = self.cursor.execute(
                "UPDATE `auctions` SET `status` = ? WHERE `author_id` = ?", (status, author_id,)
            )
    def set_current_cost(self,author_id, cost):
        with self.connection:
            return self.cursor.execute(
                "UPDATE `auctions` SET `current_cost` = ? WHERE `author_id` = ?", (cost, author_id, )
            )
