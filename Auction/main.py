...
async def call_handler(call: types.CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        if "sort" in call.data:
            ...
            await bot.send_message(
                chatid,
                "Напишите никнейм пользователя, аукцион которого вы хотите удалить. Учитывайте, что после отправки ника аукцион будет сразу же удален",
            )
            await state.set_state(ClientState.ADMINDELETE)
        elif call.data == "cancel_admin_del":
            await delete_msg(call.message, 2)
            await state.set_state(ClientState.START)
        elif "auto_start" in call.data:
            await bot.send_message(
                chatid,
