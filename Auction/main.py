...
async def start(message: types.Message, state: FSMContext):
    if message.chat.type == "private":
        if not garantDB.user_exists(message.chat.id):
            await bot.send_message(message.chat.id, 'Для доступа к этому боту необходимо нажать на старт в нашем гарант боте.', reply_markup=types.InlineKeyboardMarkup().add(
    ...
    if 'get_auctions' in call.data:
        ...
        if db.check_active_auction(chatid):
            ...
            if current_state == "ClientState:AUCTIONOWNER":
                await bot.send_message(chatid, 'Вы уже находитесь в своем аукционе.')
            else:
                await bot.send_message(chatid, 'Ваш аукцион, ожидайте участников', reply_markup=nav.owner_actions)
        ...
        try:
            ...
            for member in members_id:
                if not member == '':
                    await bot.send_message(int(member), 'Аукцион удален. Можно расходиться', reply_markup=nav.del_auction)
            await bot.delete_message(chatid, call.message.message_id)
            await bot.delete_message(chatid, call.message.message_id - 1)
            await bot.send_message(chatid, 'Аукцион успешно удален!', reply_markup=nav.action_choose)
        except Exception as e:
            print(e, call.data)
        ...
        try:
            ...
            if auction_info[2] < auction_info[5]:
                await bot.send_message(chatid, 'Уже нельзя поменять начальную ставку.')
                return
        ...
        try:
            ...
            await bot.send_message(author_id, f"К вашему аукциону присоединился новый участник \nКоличество участников: {db.get_auction_members(author_id)[0]}")
            await bot.send_message(chatid, f'Вы присоединились к аукциону №{auction_info[0]}\nТовар: {auction_info[4]}\n{"Начальная ставка" if auction_info[6] == "inactive" else "Текущая ставка"} : {auction_info[2] if auction_info[6] == "inactive" else auction_info[5]}\nУчастников: {auction_info[1]}', reply_markup=nav.member_actions)
        ...
        try:
            state_data = await state.get_data()
            author_id = state_data['auction_id']
            await bot.send_message(chatid, 'Вы вышли из аукциона',reply_markup=nav.action_choose)
            await state.update_data(auction_id=None)
            db.change_members(author_id, -1)
            current_members_id = db.get_members_id(author_id)[0]
            ...
            db.set_members_id(author_id, new_members_id)
            await bot.send_message(author_id, f'Участник вышел из аукциона. Количсетво участников: {db.get_auction_members(author_id)[0]}')
        except Exception as e:
            print(e, call.data)
            await bot.send_message(chatid, 'Что-то пошло не так.')
        ...
        try:
            ...
            if db.get_auction(author_id)[6] == 'inactive':
                ...
                await bot.send_message(chatid, f'Напишите вашу ставку. Минимальный шаг: 1 USDT. Ваш баланс: {db.get_user(chatid)[1]} USDT')
        ...
        try:
            if db.get_auction_members(chatid)[0] > 1:
                ...
                await bot.send_message(chatid, 'Аукцион начат. Участники могут делать ставки.')
                members_id = db.get_members_id(chatid)[0].split('/')
                for member in members_id:
                    if not member == '':
                        await bot.send_message(int(member), 'Аукцион начат. Можно предлагать ставки', reply_markup=nav.member_actions)
            else:
                await bot.send_message(chatid, 'Недостаточно пользователей для начала аукциона. Необходимо 2 и более участников')
        except Exception as e:
            print(e, call.data)
            await bot.send_message(chatid, 'Что-то пошло не так.')
    elif 'return' in call.data:
        await bot.send_message(chatid, 'Выбирайте действие', reply_markup=nav.action_choose)
        await state.update_data(author_id=None)
    elif 'accept_offer':
        try:
            ...
            await bot.send_message(author_id, f'Вы выиграли аукцион. Ваша ставка: {auction_info[5]} USDT. Продавец предложит вам сделку в нашем гаранте', reply_markup=nav.del_auction)
            await bot.send_message(chatid, f'Ваш аукцион закончился на ставке {auction_info[5]} USDT.\nОтправьте данный текст нашему гаранту для начала сделки: `{nav.code_link(offer_link.encode())}`',parse_mode='MARKDOWN', reply_markup=nav.del_auction)
            db.del_auction(chatid)
            for member in members_id:
                if not member == '' and not int(member) == author_id:
                    await bot.send_message(int(member), 'Аукцион закончился', reply_markup=nav.del_auction)
        except Exception as e:
            print(e)
            await bot.send_message(chatid, 'Что-то пошло не так.')
...
async def createAuction(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        await bot.send_message(message.chat.id, 'Аукцион успешно создан, ожидайте участников.', reply_markup=nav.owner_actions)
        await state.set_state(ClientState.AUCTIONOWNER)
    except Exception as e:
...
async def changeStartCost(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        for member in members_id:
                    if not member == '':
                        await bot.send_message(int(member), f'Была изменена начальная ставка аукциона: №{auction_info[0]}\nТовар: {auction_info[4]}\n{"Начальная ставка" if auction_info[6] == "inactive" else "Текущая ставка"} : {auction_info[2] if auction_info[6] == "inactive" else auction_info[5]}\nУчастников: {auction_info[1]}\nСтатус: {auction_info[6]}', reply_markup=nav.member_actions)
...
async def offerRate(message: types.Message, state: FSMContext):
    try:
        ...
        if offer > user_info[1]:
            await bot.send_message(message.chat.id, f'Ставка не может быть больше баланса. Ваш баланс: {user_info[1]}')
        elif offer <= auction_info[5]:
            await bot.send_message(message.chat.id, 'Ставка не может быть меньше предыдущей.')
        else:
            members_id = db.get_members_id(author_id)[0].split('/')
            for member in members_id:
                if not member == '':
                    await bot.send_message(int(member), f'Ставка поднята до {offer}', reply_markup=nav.member_actions)
            await bot.send_message(author_id, f'Ставка поднята до {offer}', reply_markup=nav.accept_offer(message.chat.id))
