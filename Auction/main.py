...
async def getAuctions(message: types.Message, state: FSMContext):
    try:
        ...
        if auctions_markup == None:
            ...
            await state.set_state(ClientState.START)
            return
        photo = open(f"files/{desired_category}.png", "rb")
        msg = await bot.send_photo(
            chatid,
            photo=photo,
            caption="Выберите интересующий аукцион📊",
    ...
    if message.text == "Список аукционов ⚖️":
        try:
            ...
            if db.check_active_auction(chatid) or not author_id == None:
                ...
                return
            photo = open(f"files/VIBOR KATEGORII AUCTION.png", "rb")
        ...
        try:
            ...
            if not author_id == None:
                ...
                return
            photo = open(f"files/SOZDANIE AUCTION.png", "rb")
            await bot.send_photo(
                chatid,
                photo=photo,
        ...
        try:
            if db.check_active_auction(chatid):
                photo = open(f"files/MOI AUCTION.png", "rb")
                await bot.send_photo(
                    chatid,
                    photo=photo,
                    caption="Ваш аукцион, ожидайте участников👥",
